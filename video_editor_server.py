...
def process_video(session_id, ref_path, raw_path, output_path):
    try:
        ...
        jobs[session_id]['message'] = f'Reference: {num_ref_clips} cuts detected, avg {avg_clip:.1f}s per clip'
        ...
        for i, start in enumerate(clip_starts):
            ...
            cmd = ['ffmpeg', '-y', '-ss', f'{start:.3f}', '-i', raw_path,
                   '-t', f'{clip_dur:.3f}', *vf_args,
        ...
        jobs[session_id].update(status='done',
...
def upload():
    ...
    ref_file.save(os.path.join(session_dir, f'reference{ref_ext}'))
    raw_file.save(os.path.join(session_dir, f'raw{raw_ext}'))
...
def process(session_id):
    ...
    t = threading.Thread(target=process_video, daemon=True,
        args=(session_id, os.path.join(session_dir, ref_files[0]),
              os.path.join(session_dir, raw_files[0]),
